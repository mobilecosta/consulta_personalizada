...
oQuery := TCQuery():New(cPQ1_ID, cDebug == "1", cPQ6_ORDEM)
oCQuery := oQuery
...
//-- Verifica evento criaÁ„o tabela
If ! Empty(cPQ6_ORDEM)
	U_ExecMemo(cPQ1_ID, cPQ6_ORDEM, .F.)
EndIf
...
While PQ6->PQ6_ID == cPQ1_ID .And. ! PQ6->(Eof())
	...
	if lInsert
		...
		If PQ6->PQ6_TIPO <> "5"	// CriaÁ„o do Formulario
...
If nPos > 0 .And. ! Empty(aQuery[nPos][2]:cPQ1_SX2) .And. aQuery[nPos][2]:cPQ1_SX2 <> aQuery[nPos][2]:cAlias
