...
<Data> : 02/09/2013
<Parametros> : cAlias = Tabela do dicionário a ser iniciada
<Retorno> : Nenhum
...
Montagem do relatório a partir do dicionário de dados da consulta personalizada
...
If cPQ1_ID = Nil .Or. ValType(cPQ1_ID) <> "C"
	Alert("Atenção. O Id do relatório não foi enviado !")
	Return
EndIf
...
±±ºPrograma  ³ LockByFl  º Autor ³ Fernando Salvatori º Data ³ 10/09/2013 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Realiza um Lock via Arquivo                                º±±
±±º          ³                                                            º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³UnLockByFl º Autor ³ Fernando Salvatori º Data ³ 10/09/2013 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Realiza UnLock via Arquivo                                 º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ ACCSTA13                                                   º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para geração de integração com Rest a partir de formulário da consulta
...
Retorna as configurações para consumo da API do MarketPlace
...
@since 12/05/2017
@version 1.0
@param cEmpAdq, character, Empresa de Integração
...
User Function TGtTknAqds( cEmpAdq , cProcesso , cToken , cRetWsErro )
	...
	//-----------------------------------------------------------------------
	// Verifica se o Metodo a Ser consumido precisa de autenticação
	//-----------------------------------------------------------------------
	If PN4->( dbSeek( FWxFilial("PN4") + Padr( cEmpAdq , TamSx3("PN4_CINTEG")[1] ) + Padr( cProcesso , TamSx3("PN4_PROCES")[1] ) ) )
...
@since 17/05/2017
@version 1.0
@param aRest, array, (Descrição do parâmetro)
...
@param cMsgVld, character, Msg de validação, caso ocorra erros
...
@param aHeadAPI, array , Arary com parametros para complementar o Header da requisição
...
User Function FCsmWSAdq(cAlias, xParam, cCodEmp, cProcesso, cComplURL, cMsgVld, cToKenExt, cRetWsPad, cRetWsErro, aHeadAPI,;
	...
	Else
		...
		Else
			...
			ElseIf cTpMtd == "5" // Para o metodo PATCH, ( Metodo PATCH não implementado no FWRest ainda )
		...
		If cTpMtd <> "5" // Metodo PATCH não implementado no FWRest ainda
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Definições da estrutura do relatório
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Montagem da seção do relatório a partir do ID da consulta personalizada
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Impressão da estrutura do relatório
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Retorna expressão para aplicação do filtro

@author Wagner Mobile Costa
@version P12
@param = cAlias = Alias para montagem da expressão
         cJoin = cJoin = Join no formato string como E1_CLIENTE=A1_COD;E1_LOJA=A1_LOJA 
         lMaster = Expressão de filtro para o cabecalho ou detalhe
@return = Expressão para o comando:
...
Local cFilter := cAlias + "->(", aJoin := U_RetJoin(cJoin)
Local nJoin := 1 
Local nPos  := If(lMaster, 2, 1)
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Execução de job a partir da consulta personalizada
...
If cPQ1_JOB == "1"	// Percorre cada registro da consulta e executa ação
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para importação de arquivo CSV e inserção via SQL, RecLock ou Rotina Automatica
...
Local cDir	  := ""
Local cTitulo := ""
Local cDesc1  := "Esta rotina permite a importação de arquivo no formato CSV para o banco de dados"
...
If Empty(_cArquivo)
	Aviso("Atenção", "A pasta com os arquivos a serem importados não foi escolhido !", {"Fechar"}, 3)
	Return
EndIf
...
If ! Empty(cFields)
	MsgInfo("Atenção. O(s) campo(s) [" + cFields + "] não foram localizados no dicionário do formulário [" +;
...
While !FT_FEOF()
	...
	For nLinha := 1 to Len(aFields)
		...
		// Tratamento para campo imagem
		// 1=Caracter;2=Numerico;3=Logico;4=data;5=Memo
		
		If "BITMAP" $ aFields[nLinha, 1]
			_xDado := DlgPutImg( _xDado )
			lBitMap := .T.
		ElseIf At("<ADVPL>", _xDado)
	...
	If nTypeImp == 1
		...
		IF ! Empty(cError)
			...
			EndIf
			AutoGrLog("Linha: " + AllTrim(Str(_nCt)) + "-Instrução " + cSql + " - Erro: " + cError)
		EndIf
	ElseIf nTypeImp == 2	//-- RecLock
		(__ALIAS)->(MsUnLock())
	EndIf
...
Local nExec  := 0
	...
	Do While .T.
		...
		if n2 == 0 .and. n1 > 0
			Alert("Instrução Advpl [" + cStmt + "] Estrutura sem a tag </ADVPL>!")
			break
		endif
		...
		if ValType(cLinha) == "C"
			...
			nExec ++
		ElseIf nExec > 1
			Alert("Atenção. O parametro [" + cParam + "] não está sendo substituido na Pre-SQL [" + cStmt + "] ")
			Break
		endif
	EndDo
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para importação de arquivo texto para tabela PN1
...
Local cDir	  := ""
Local cTitulo := ""
Local cDesc1  := "Esta rotina permite a importação de arquivo texto"
Local cDesc2  := "É obrigatório escolher a pasta para leitura no botão [Parametros]."
Local cArqDes := CriaTrab(nil, .f.) + ".txt"
Local cMsg    := cTime := ""
Local nReg	  := 0
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para processamento das requisições na PN1
...
If ! U_TGtTknAqds( cPN1_CINTEG, cPN1_PROCES, "" , @cRetWsErro )
	Alert("Erro ao realizar requisição para comunicação")
	Return
EndIf
__CONTINUA := .T.
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para exportação de arquivo CSV a partir do retorno REST da tabela PN1
...
/*/
//-----------------------------------------------------------------------------
User Function TRGPN1CSV(cAlias, aJoin, cFilter, cAOX_ROTINA)
Local aRet	  := {}
Local aSay    := {}
Local aButton := {}
Local nOpc    := 0
Local cDir	  := ""
Local cTitulo := ""
Local cDesc1  := "Esta rotina permite a exportação do retorno do REST para arquivo CSV"
Local cDesc2  := "É obrigatório escolher a pasta para gravação no botão [Parametros]."
...
If Empty(_cArquivo) .Or. Upper(Right(_cArquivo, 4)) <> '.CSV'
	Aviso("Atenção", "Deve ser informada a pasta e o nome do arquivo CSV !", {"Fechar"}, 3)
	Return
EndIf
...
IF !":\" $ _cArquivo
	Aviso("Atenção", "O arquivo selecionado deve estar em um drive existente na máquina local." + CRLF + CRLF + ;
		"Não será possível a importação de arquivos a partir do servidor!", {"Fechar"}, 3)
ELSE
	MsAguarde({|| ProcCSV(_cArquivo, cArqDes, cAlias, aJoin, cFilter, cAOX_ROTINA)  },"Aguarde...","Gerando Arquivo [" + _cArquivo + "]")
	FErase(cArqDes)
ENDIF
...
Static Function ProcCSV(_cArquivo, cArqDes, cAlias, aJoin, cFilter, cAOX_ROTINA)
...
BeginSQL Alias "QRY"
	SELECT AOX_CAMPO, AOX_REGRA1
	  FROM %Table:AOX%
	 WHERE AOX_FILIAL = %Exp:xFilial("AOX")% AND AOX_ROTINA = %Exp:cAOX_ROTINA% AND D_E_L_E_T_ = ' '
	 ORDER BY AOX_XITEM
EndSQL
...
if nHandle == -1
	MsgAlert("Não foi possivel criar o arquivo de Saida.", "Atenção")
	return
endif
...
If FWRITE(nHandle, cLinha, len(cLinha)) != len(cLinha)
	MsgAlert("Ocorreu um erro na gravação do arquivo <Csv>,", "Atenção")
	return
Endif
...
EndIf
	...
	If LastRec() > 1 .And. ! IsBlind()
		...
		IF ! ParamBox(aParam, "Seleção da Empresa",, {|| AllwaysTrue()},,,,,,, .F.)
			Return .F.
		Endif
		SM0->(DbSeek(mv_par01))
		cOEmp := SM0->M0_CODIGO
...
//-----------------------------------------------------------------------------
/*/ {Protheus.doc} Function
Rotina para execução de select e retorno de campo
...
?un‡…o    ?lgPutImg     ?utor?arinaldo de Jesus   ?Data ?3/05/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?mportar as Fotos dos Funcionarios                          ?
...
Static Function DlgPutImg( cFile )
...
/*/
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ?
?un‡…o    ?utImg        ?utor?arinaldo de Jesus   ?Data ?3/05/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?ravar a Imagem no Repositorio de Imagens e Vincula-la    ao?
?         ?uncionario													?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?intaxe   ?vide parametros formais>									?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
...
?so       ?enerico                      								?
ÀÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?*/
Static Function PutImg( oRepository , cPathFile )
	...
	Begin Sequence
		...
		cFile := RetFileName( cPathFile )

		oRepository:InsertBmp( cPathFile , NIL , @lPut )
...
/*/
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ?
?un‡…o    ?oveFile	  ?utor?arinaldo de Jesus   ?Data ?3/05/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?over as Imagens que foram importadas para o Repositorio   e?
?         ?inculadas ao Funcionario para um diretorio "BackUp"		?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?intaxe   ?vide parametros formais>									?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?arametros?vide parametros formais>									?
...
/*/
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ?
?un‡…o    ?qualFile     ?utor?arinaldo de Jesus   ?Data ?9/03/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?erifica se Dois Arquivos sao Iguais                        ?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?intaxe   ?vide parametros formais>									?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?arametros?vide parametros formais>									?
...
/*/
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ?
?un‡…o    ?irMake       ?utor?arinaldo de Jesus   ?Data ?9/03/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?ria um Diretorio                                           ?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?intaxe   ?vide parametros formais>									?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?arametros?vide parametros formais>									?
...
/*/
ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ?
?un‡…o    ?ileMove      ?utor?arinaldo de Jesus   ?Data ?9/03/2005?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ?
?escri‡…o ?over um arquivo de Diretorio                               ?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?intaxe   ?vide parametros formais>									?
ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
?arametros?vide parametros formais>									?
